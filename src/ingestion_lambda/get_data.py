...
def get_data(conn, last_fetch_ending_time=dt.datetime(2020, 1, 1)):
    '''get data from the connected database from last_fetch_ending_time
    params:
        conn: database connection instance
            type: pg8000 connect object
        last_fetch_ending_time: the timestamp when the last fetch data end up with
    ...
    try:
        ...
        for table in table_names:
            ...
            content = conn.run(
                f"SELECT * FROM {table[0]} WHERE created_at>:date or last_updated>:date;", date=last_fetch_ending_time)

            # get all column names from the table
            columns = conn.run(
                f"SELECT column_name from information_schema.columns WHERE table_schema='public' AND table_name= '{table[0]}'")
